...
async def handle_msg(update: Update, context: CallbackContext):
    ...
    for qid, qname in QUALITY_OPTIONS.items():
        row.append(InlineKeyboardButton(qname, callback_data=f"dl_quality|||{qid}|||{quote(url, safe='')}"))
        if len(row) == 2:
            kb.append(row)
            row = []
    if row:
...
async def audio_callback(update: Update, context: CallbackContext):
    ...
    await do_download(update, context, url, height_str=None, audio_only=True)
